...
StoreReport::StoreReport()
{
	...
	//2.加载配置
	std::string db_host = config_file::instance()->GetString("mysql", "host", "127.0.0.1");
    short db_port = config_file::instance()->GetNumber("mysql", "port", 3306);
    std::string db_user = config_file::instance()->GetString("mysql", "user", "root");
    std::string db_passwd = config_file::instance()->GetString("mysql", "passwd", "123");
    std::string db_name = config_file::instance()->GetString("mysql", "name", "lrlb_dns");
...
void StoreReport::store(lrlb :: ReportStatusReq req)
{
	for(int i=0;i<req.results_size();i++){
		//一条report调用记录
		const lrlb::HostCallResult& result=req.results(i);
		...
		mysql_ping(&_db_conn); //ping测试一下，防止连接断开，会触发重新建立连接
