...
int main() 
{
    ...
    std::string ip = config_file::instance()->GetString("reactor", "ip", "192.168.237.128");
    short port = config_file::instance()->GetNumber("reactor", "port", 8888);
	int phread_num=config_file::instance()->GetNumber("reactor", "threadNum", 3);
    printf("ip = %s, port = %d\n", ip.c_str(), port);

    tcp_server server(&loop, ip.c_str(), port,phread_num);
