...
void Usage() {
    printf("Usage: ./lrlb_dns_test -h ip -p port\n");
}
...
void on_connection(net_connection *conn, void *args) 
{
    ...
    conn->send_message(requestString.c_str(), requestString.size(), lrlb::ID_GetRouteRequest);
}
...
void deal_get_route(const char *data, uint32_t len, int msgid, net_connection *net_conn, void *user_data)
{
    ...
    std::string requestString;
    req.SerializeToString(&requestString);
    net_conn->send_message(requestString.c_str(), requestString.size(), lrlb::ID_GetRouteRequest);
}
...
int main(int argc, char **argv)
{
    ...
    //创建客户端
    client = new tcp_client(&loop, option.ip, option.port, "lrlb_dns_test");
    if (client == NULL) {
    ...
    //设置服务端回应包处理业务
    client->add_msg_router(lrlb::ID_GetRouteResponse, deal_get_route);
