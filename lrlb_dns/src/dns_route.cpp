...
// return 0表示加载成功，version没变，1加载成功，version变了，-1是加载失败
...
void Route::load_changes(std::vector<uint64_t>& change_list){
    ...
    if (lineNum == 0)
    {
		...
		printf("has a new change in changelong\n");
	}
    MYSQL_ROW row;
	...
	mysql_free_result(result);
	printf("point 2 ,finish the load changes\n");
}
...
{
	//10s自动修改一次，也可以从配置文件读取
	int wait_time=config_file::instance()->GetNumber("Route", "time",10);
	...
	while(true){
		...
		//1.1加载RouteVersion得到当前版本号
		int ret=Route::instance()->load_version();
		if(ret==1) //加载成功，并且有修gai
		{
			//2.1将最新的RouteData加载到_temp_pointer中 
			if(Route::instance()->load_route_data()==0){
				//2.2交换指针
				Route::instance()->swap();
				last_load_time=time(NULL); //跟新最后加载的时间
			}
			//2.3 获取别修改modid/cmdid对应订阅的客户端，并进行推送
			...
			if(current_time-last_load_time>=wait_time){  //超时
				if(Route::instance()->load_route_data()==0){
				//3.1交换指针
				Route::instance()->swap();
				last_load_time=time(NULL); //跟新最后加载的时间
